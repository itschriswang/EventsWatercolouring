...
// varied, watercolour-brush underline rather than a straight rule.
const PATHS = [
...
// Fixed for the life of the page load, but changes minute to minute — so a
// keyword painted the same way on one visit reads with a different brush
 ...
 * usual "draw on hover" pattern — and it redraws on mouse-out. Reduced-motion
 * gets a static underline with no animation.
 */
 ...
 */
export function withUnderline(text, phrase, extraProps = {}) {
  const i = text.indexOf(phrase)
  if (i === -1) return text
  return [
    text.slice(0, i),
    <Underline key="u" {...extraProps}>{phrase}</Underline>,
