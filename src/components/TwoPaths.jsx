...
export default function TwoPaths() {
  ...
  return (
    <section
      id="process"
      className="relative w-full overflow-hidden bg-wine px-[5vw] py-[clamp(4rem,8vw,7rem)] text-paper"
    >
      ...
      <div className="relative z-10 mt-[clamp(2.5rem,6vw,5rem)] grid grid-cols-12 gap-x-8 gap-y-16">
        {PATHS.items.map((path, i) => (
          ...
          >
            ...
            <h3 className="mt-3 font-sentient text-[clamp(2rem,4.5vw,4rem)] font-bold leading-[0.95] tracking-[-0.02em] text-paper">
              {path.title}
            </h3>
            ...
            <ol className="mt-8 flex flex-col gap-6 border-t border-paper/15 pt-8">
              {path.steps.map((s, si) => (
                <li key={si} className="flex gap-5">
                  ...
                  <span>
                    <b className="block font-sentient text-lg font-bold tracking-[-0.01em] text-paper">
                      {s.b}
                    </b>
