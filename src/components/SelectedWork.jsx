    ...
    <section id="work" className="relative w-full px-[5vw] py-[clamp(3rem,7vw,7rem)]">
      <div className="mx-auto max-w-[88rem]">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <Label gradient={['#C6F04A', '#FF5A3C']}>{WORK.label}</Label>
            <SplitText
              as="h2"
              unit="char"
              lines={WORK.title}
    ...
    >
      {item.testimonial ? (
        <div className={cardShape + ' bg-paper-deep'}>
          <CornerBloom from="rgba(224,168,30,0.14)" to="rgba(228,136,156,0.11)" />
        ...
        >
          ...
          <CornerBloom from="rgba(255,90,60,0.14)" to="rgba(228,136,156,0.10)" overlay />
        </button>
      )}
    </motion.figure>
  )
        ...
        >
          ...
          {dressed && (
            <motion.div
              ...
              style={{
                background:
                  'radial-gradient(circle at 50% 50%, rgba(255,90,60,0.42), rgba(228,136,156,0.2) 42%, transparent 72%)',
                filter: 'blur(60px)',
              }}
            />
          )}
