...
const FRAG = `
  ...
  vec3 pigment(float t, float warm){
    vec3 terracotta = vec3(0.760, 0.380, 0.235);
    vec3 ochre      = vec3(0.788, 0.635, 0.227);
    vec3 blush      = vec3(0.894, 0.533, 0.612);
    vec3 rose       = vec3(0.788, 0.545, 0.549);
    vec3 lime       = vec3(0.682, 0.749, 0.337);
    vec3 cool       = mix(vec3(0.431, 0.549, 0.659), vec3(0.682, 0.749, 0.337), warm);
    vec3 rust       = vec3(0.643, 0.314, 0.184);
    t = fract(t) * 6.0;
